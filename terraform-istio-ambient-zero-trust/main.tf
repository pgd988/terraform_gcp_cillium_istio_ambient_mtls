 ...
 * 4. `ztunnel` Helm chart release (L4 eBPF / socket redirection on Dataplane V2).
 * 5. Kubernetes resources child module (`./kubernetes`) for backbone STRICT mTLS and root CA synchronization (`cacerts` ExternalSecret).
 *
...
# 5. Invoke Kubernetes Resources Submodule (Backbone STRICT mTLS and Root CA ExternalSecret)
module "kubernetes" {
  source = "./kubernetes"
  count  = var.enable ? 1 : 0
